...
class Hist(torch.autograd.Function):
	...
	@staticmethod
	def forward(ctx,x,sigma,n_bins,func):
		M, N = x.shape
		noise = sigma*torch.randn(x.shape).double()
		#print(noise)
		noisy_input = torch.clamp(x+noise,0,n_bins-1)	# add noise to input and clamp		
		#print('inputs:',x,noisy_input)
		labels = torch.stack(func.neighs(noisy_input))	# This stack gives P 1D neighbors for each dimension
		P = labels.shape[0]
...
def hist_brute_force(x,sigma,n_bins,func):
	M,N = x.shape
...
if __name__ == '__main__':
	...
	hist_list = [
		...
		lambda x: hist_brute_force(x,0,n_bins,interp),
		lambda x: Hist.apply(x,0,n_bins,interp)
	]
